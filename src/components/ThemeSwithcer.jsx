...
const ThemeSwitcher = () => {
  ...
  return (
    <div 
      className="flex h-8 w-14 items-center justify-around rounded-full bg-[#2e2f33] dark:bg-black cursor-pointer duration-200 transform"
      onClick={() => theme === 'light' ? toggleTheme("dark") : toggleTheme("light")}
    >
      <button
        className={`flex justify-center items-center ${activeCircleStyles} ${theme === "light" ? "bg-white" : ""}`}
      >
        <svg
          width="14"
      ...
      <button
        className={`flex justify-center items-center ${activeCircleStyles} ${theme === "dark" ? "bg-white" : ""}`}
      >
        <svg
