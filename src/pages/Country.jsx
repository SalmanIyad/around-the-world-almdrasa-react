...
const Country = () => {
  ...
  if (result?.name?.common === "Israel"){
    // navigate("/404")
    result.capital="Al-Hawiah الهاوية"
    result.region="Jahanam جهنم"
    result.subregion="Al-Jaheem الجحيم"
    result.tld=[".jhnm"]
    console.log(result.currencies);
    result.currencies={"Unknown": {name: "زقوم", symbol: ""}}
    result.flags.alt="The Ki11ers"
    result.languages=["Unknown"]
  }
  if (result?.name?.common === "Palestine"){
    result.capital = "Jerusalem القدس عاصمة فلسطين الأبدية"
  }
  ...
  return (
    <>
      ...
      {!isError && !isLoading && (
        <div>
          <Link
            className="mb-16 inline-block rounded-md bg-white dark:bg-gray-400 p-3 md:mb-20"
            to="/"
          >
            <svg
              width="19"
                    ...
                    <span>
                      {result?.currencies &&
                        Object.keys(result.currencies)
                          .map((currency) => `${result?.currencies[currency].name} (${result?.currencies[currency].symbol})`)
                          .join(", ")}
                    </span>
                  </p>
